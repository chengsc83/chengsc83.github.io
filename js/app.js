        const App = {
            themes: {
                default: { name: '經典靛藍', colors: { accent: '#4f46e5', accentHover: '#4338ca', accentLight: '#6366f1' } },
                rose: { name: '熱血玫瑰', colors: { accent: '#e11d48', accentHover: '#be123c', accentLight: '#f43f5e' } },
                amber: { name: '活力暖陽', colors: { accent: '#d97706', accentHover: '#b45309', accentLight: '#f59e0b' } },
                teal: { name: '清新青藍', colors: { accent: '#0d9488', accentHover: '#0f766e', accentLight: '#14b8a6' } },
            },
            // --- STATE ---
            state: {
        ...
        `;
            ...
            actions: {
                ...
                deleteCustomFlow(e) {
                    const flowSelect = document.getElementById('formatSelect');
                ...
                formatChanged(e) {
                    ...
                    if (previewContainer) {
                        ...
                        if (flow && flow.length > 0) {
                            ...
                            previewContainer.innerHTML = '<span class="format-preview-tag">請選擇賽制或建立自訂流程</span>';
                            previewContainer.style.display = 'flex';
                        }
                    }
                },
            ...
            renderSetupView() {
                ...
                <div class="setup-container w-full max-w-4xl mx-auto animate-fade-in-up px-4 md:px-0 pb-32">
                    ...
                    <div class="flex items-center justify-center gap-2 mb-8">
                        <button data-action="goToSetupStep" data-step="1" class="flex items-center gap-2 transition-all ${currentStep === 1 ? '' : 'opacity-80 hover:opacity-100'}">
                        ...
                        <button data-action="goToSetupStep" data-step="2" class="flex items-center gap-2 transition-all ${currentStep === 2 ? '' : 'opacity-80 hover:opacity-100'}">
                        ...
                        <button data-action="goToSetupStep" data-step="3" class="flex items-center gap-2 transition-all ${currentStep === 3 ? '' : 'opacity-80 hover:opacity-100'}">
                        ...
                        <div class="glass-panel p-6 md:p-8 relative overflow-hidden">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <!-- 標籤篩選區域 -->
                                    <div class="mb-3">
                                        <div id="formatTagsContainer" class="flex flex-wrap gap-2">
                                    ...
                                    <div class="flex gap-2">
                                        <div class="searchable-dropdown relative flex-grow" id="formatDropdownContainer">
                                            <!-- 搜尋輸入框 (也是觸發器) -->
                                            <div class="relative">
                                                <input type="text"
                                                    id="formatSearchInput"
                                                    class="modern-input pl-10 pr-10 w-full text-sm cursor-pointer"
                                                    placeholder="點擊選擇或搜尋賽制..."
                                                    autocomplete="off">
                                                <div class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                                                </div>
                                                <div class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 cursor-pointer hover:text-slate-600 transition-all duration-200" id="formatDropdownArrow" data-action="toggleFormatDropdown">
                                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                                                </div>
                                            </div>

                                            <!-- 下拉選項列表 -->
                                            <div id="formatDropdownList" class="searchable-dropdown-list hidden py-2 bg-[var(--surface-1)] border border-[var(--border-color)] rounded-xl shadow-xl max-h-72 overflow-y-auto" style="z-index: 99999 !important;">
                                                <!-- 選項會由 JS 動態生成 -->
                                            </div>

                                            <!-- 隱藏的原生 select 用於保持相容性 -->
                                            <select id="formatSelect" data-change-action="formatChanged" class="hidden">
                                                ${formatOptions}
                                            </select>
                                        </div>
                                        <button data-action="deleteCustomFlow" title="刪除自訂流程" class="flex-shrink-0 w-11 h-11 rounded-xl border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                        </button>
                                    </div>

                                    <div id="formatFilterInfo" class="text-xs text-slate-400 mt-2" style="display:none;"></div>
                        ...
                        <details class="glass-panel overflow-hidden group" open>
                            ...
                            <div class="px-5 pb-5 pt-2 border-t border-[var(--border-color)] space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-[var(--border-color)]">
                                    ...
                                    <div class="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                        <span class="text-sm font-medium">緩衝秒數</span>
            ...
            <div class="debate-layout grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 h-full pb-28">
                ...
                <div class="lg:col-span-8 flex flex-col gap-4 h-full">
                    <!-- 頂部資訊列 -->
                    <div id="debateInfoContainer" class="grid grid-cols-1 md:grid-cols-2 gap-3"></div>

                    <!-- 比賽流程追蹤器 -->
                    <div id="debateFlowTracker" class="debate-flow-tracker"></div>
                ...
                <!-- 逐字稿面板 -->
                <div class="lg:col-span-4 h-full min-h-[280px] hidden lg:block">
        ...
        `;
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full pb-24">
                <div class="lg:col-span-8 flex flex-col gap-6 h-full">
                    ...
                    <div id="debateInfoContainer" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="lg:col-span-4 h-full min-h-[300px]">
                    <div id="transcriptionPanel" class="glass-panel transcription-card overflow-hidden h-full"></div>
                </div>
            </div>
            <div id="debateControlsContainer"></div>
        ...
        `;
            ...
            renderTranscriptionPanel() {
                ...
                const { isRecording, isAvailable } = this.state.recording;

                container.innerHTML = `
            <div class="flex flex-col border-b border-[var(--border-color)] bg-[var(--surface-2)] backdrop-blur-md sticky top-0 z-10">
                ...
                <div class="px-4 py-3 flex justify-between items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold text-sm">會議紀錄</span>
                    </div>
                    ${isRecording ? '<span class="text-xs font-mono text-red-500 animate-pulse flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-red-500"></span> REC</span>' : ''}
                </div>

                <div class="px-4 pb-3 grid grid-cols-2 gap-3">

                    <button data-action="${isRecording ? 'stopRecording' : 'startRecording'}"
                            ${!isAvailable ? 'disabled' : ''}
                            class="btn-record-toggle flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${isRecording ? 'bg-red-500 border-red-600 text-white shadow-inner hover:bg-red-600' : 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-600'}">
                        ${isRecording ?
                        `<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h12v12H6z"/></svg> 停止錄音` :
                        `<span class="w-2 h-2 rounded-full bg-red-500"></span> 開始錄音`
                    }
                    </button>

                    <div class="transcription-ctrl-group flex bg-slate-100 dark:bg-slate-700/50 rounded-lg p-1 shadow-inner border border-transparent dark:border-slate-600/50">
                        ${!active ?
                        `<button data-action="startTranscription" class="w-full rounded-md bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] text-white text-xs font-bold py-1.5 transition-colors shadow-sm">開啟轉錄</button>` :
                        `
                            ${!paused ? `<button data-action="pauseTranscription" class="flex-1 rounded-md text-amber-600 dark:text-amber-500 hover:bg-white dark:hover:bg-slate-600 text-xs font-bold py-1 transition-all">暫停</button>` : ''}
                            ${paused ? `<button data-action="resumeTranscription" class="flex-1 rounded-md text-[var(--color-primary)] hover:bg-white dark:hover:bg-slate-600 text-xs font-bold py-1 transition-all">繼續</button>` : ''}
                            <div class="w-[1px] bg-slate-300 dark:bg-slate-600 mx-1 my-2"></div>
                            <button data-action="stopTranscription" class="flex-1 rounded-md text-red-500 hover:bg-white dark:hover:bg-slate-600 text-xs font-bold py-1 transition-all">停止</button>
                            `
                    }
                    </div>
                </div>
            ...
            <div id="paragraphsContainer" class="p-4 flex-grow overflow-y-auto scroll-smooth space-y-4"></div>

            <div class="p-3 border-t border-[var(--border-color)] bg-[var(--surface-1)] text-sm min-h-[3.5rem]">
                <div class="flex">
        ...
        `;
            ...
            renderTranscriptionParagraphs() {
                ...
                // 只有當使用者在底部附近時才自動滾動，避免閱讀時被打斷 (這裡簡單處理，強制滾動)
                requestAnimationFrame(() => {
            ...
            renderModal({ title, body, footer }) {
                ...
                // UX 優化 2：將 z-index 設為最高，確保浮在所有介面之上
                ...
                <div class="relative glass-panel rounded-2xl shadow-2xl max-w-md w-full overflow-hidden transform transition-all scale-100 animate-scale-in border border-white/10">
                    ...
                    <div class="p-6 max-h-[60vh] overflow-y-auto text-[var(--text-main)]">${body}</div>
                    <div class="p-4 modal-footer flex justify-end gap-3 bg-[var(--surface-1)] border-t border-[var(--border-color)]">
                        ${footer || `<button data-action="closeModal" class="px-4 py-2 rounded-lg bg-[var(--surface-2)] hover:bg-[var(--border-color)] transition-colors font-medium text-sm">關閉</button>`}
                    </div>
                </div>`;
                        ...
                        <div class="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <div class="flex gap-4 md:gap-6">
                                ...
                                <div class="text-center px-4 py-2 rounded-xl bg-[var(--surface-2)] border border-[var(--border-color)]">
                                    ...
                                    <div class="text-xs text-slate-500">預估總時長</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            renderStageEditorModal(stage = {}, index = NaN) {
                ...
                const renderDynamicFields = (type) => {
                    ...
                    if (['manual_prep', 'speech_auto', 'choice_speech', 'free_debate'].includes(type)) {
                        mainSettings += `<div><label class="block text-sm font-medium mb-1">主要時長 (秒)</label><input name="duration" type="number" class="${inputClasses}" value="${current.duration || ''}"></div>`;
                    }
                    ...
                    if (choiceSettings) html += createSection('選擇性發言設定', choiceSettings);
            ...
            `;
                ...
                <div class="p-4 text-center text-xs text-slate-500 border-t border-[var(--border-color)]">Debate Clock Pro v2.0 <br> Designed for Debaters.</div>
            `;
            },
            ...
            sendProjectorUpdate() {
                ...
                try {
                    ...
                    this.state.projector.channel.postMessage({
                        type: 'TIMER_UPDATE',
                        payload: {
                            timeRemaining: timer.timeLeft || 0,
                            isRunning: !!(timer.interval || timer.graceInterval),
                            currentStage: currentStageName,
                            totalTime: timer.initialDuration || 0,
                            isTimesUp: isTimesUp,
            ...
            async startAudioDetection(team) {
                ...
                if (!mode) {
                    console.warn(`No audio source mode found for team: ${team}. Aborting.`);
                    return;
                }
                ...
                if (this.state.audioDetection.isActive || !this.state.enableSpeechDetection) return;

                this.stopAudioDetection(); // Clean up previous instance
                ...
                try {
                    ...
                    if (mode === 'display') {
                        ...
                        // 從已儲存的音訊軌道建立一個新的 MediaStream 給 VAD 使用
                        stream = new MediaStream([this.state.sharedDisplay.audioTrack.clone()]);
                    } else {
                        stream = await navigator.mediaDevices.getUserMedia({ audio: { sampleRate: 16000 } });
                    }

                    if (!stream.getAudioTracks().length) {
                        this.showNotification("選擇的來源沒有音訊，無法進行偵測。", "error");
                    ...
                    const myvad = await vad.MicVAD.new({
                        stream: stream,
                        onSpeechStart: () => {
                            // [FIX START] --- 加入保護機制，防止重複觸發 ---
                            // 如果計時器已經因為語音偵測而啟動，就直接返回，不再執行後續操作
                            if (this.state.mainSpeechTimerStartedByGrace) {
                                return;
                            }
                            // [FIX END]

                            const { timer } = this.state;
                            if (timer.type === 'grace' && timer.graceInterval && !timer.isPaused) {
                                this.state.mainSpeechTimerStartedByGrace = true; // ★★★ 立刻設定旗標，防止重複進入
                    ...
                    this.showNotification("無法啟動語音偵測，請檢查權限或裝置。", "error");
                    this.stopAudioDetection();
                }
            },
            ...
            stopAudioDetection() {
                ...
                // 增加防禦性檢查：確認 vad 物件存在，且 .destroy 是一個函式
                ...
                } else if (audioDetection.vad !== null) {
                    // 只有當 vad 物件存在但沒有 destroy 方法時才記錄 (改用 debug 級別避免干擾)
                    console.debug("VAD object did not have a callable .destroy function. VAD object:", audioDetection.vad);
                }

                // 這是確保麥克風圖示關閉的關鍵程式碼，無論如何都要執行
                ...
                console.debug("VAD (Voice Activity Detection) cleanup process completed.");
            },
            audioCheckLoop() {
                const { audioDetection, timer, mainSpeechTimerStartedByGrace } = this.state;
                if (!audioDetection.isActive || !audioDetection.analyser) return;

                // Schedule next check
                audioDetection.animationFrameId = requestAnimationFrame(this.audioCheckLoop.bind(this));

                // Get audio data
                const dataArray = new Uint8Array(audioDetection.analyser.frequencyBinCount);
                audioDetection.analyser.getByteFrequencyData(dataArray);
                ...
                if (average > audioDetection.threshold) {
                    audioDetection.activationCounter++;
                } else {
                    audioDetection.activationCounter = 0;
                }

                // If volume has been consistently high, trigger the timer
                if (audioDetection.activationCounter >= audioDetection.requiredActivations) {
                    if (timer.type === 'grace' && timer.graceInterval && !mainSpeechTimerStartedByGrace && !timer.isPaused) {
                        // This is the trigger!
                        this.stopAudioDetection(); // Stop listening once triggered
            ...
            getTimerState(timeLeft, initialDuration) {
                // 危急狀態：剩餘 10 秒以下（總時長需大於 30 秒才觸發）
                if (initialDuration > 30 && timeLeft <= 10) return 'danger';
                // 警告狀態：剩餘 30 秒以下（總時長需大於 60 秒才觸發）
            ...
            updateTimerDisplay(time) {
                ...
                if (timerEl) {
                    ...
                    // [優化] 只在狀態變化時才更新類別
                    let newClass = null;
                    if (this.state.timer.initialDuration > 30 && time <= 30) newClass = 'text-red-500';
                    else if (this.state.timer.initialDuration > 60 && time <= 60) newClass = 'text-yellow-500';
            ...
            getStageChoice(stage) {
                return new Promise((resolve, reject) => {
                    ...
                    const optionsHTML = stage.actionChoices.map(action => {
                        ...
                        if (isDisabled) {
                            return `<button disabled class="px-8 py-4 rounded-xl text-white font-bold text-lg bg-gray-400 cursor-not-allowed opacity-50">
                            ${action} (已用完)
                        </button>`;
                        }
                        return `<button data-action="confirmStageChoice" data-choice="${action}" class="px-8 py-4 rounded-xl text-white font-bold text-lg bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] transition-all hover:scale-105 shadow-lg">
                        ${action} (剩${remaining}次)
                    </button>`;
                    }).join('');
            ...
            async startRecording() {
                ...
                try {
                    ...
                    this.state.recording.mediaRecorder.onstop = () => {
                        ...
                        this._consolidateRecordingChunks();
                        const { intermediateBlobs } = this.state.recording;
                        this.state.recording.audioBlob = new Blob(intermediateBlobs, { type: 'audio/webm' });
                        // 清空中間資料
                        this.state.recording.intermediateBlobs = [];
                        this.state.recording.recordedChunks = [];
            ...
            renderPipCanvas() {
                ...
                if (stage && stage.type === 'free_debate') {
                    ...
                    ctx.fillText(positiveTeamName, halfWidth / 2, 60);
                    ...
                    ctx.font = bodyFont(28);
                    ctx.fillText(negativeTeamName, halfWidth + halfWidth / 2, 60);
